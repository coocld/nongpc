...
if($submit) {
	...
	$api_msg = $api_url = '';
	$option = isset($option) ? trim($option) : 'mobile';
	if(is_email($username) && $option == 'username') $option = 'email';
	if(!check_name($username) && $option == 'username') $option = 'mobile';
	in_array($option, array('username', 'passport', 'email', 'mobile', 'company', 'userid')) or $option = 'username';
	$r = $db->get_one("SELECT username,mobile FROM {$DT_PRE}member WHERE `$option`='$username'");

	if($r) {
		$username = $r['username'];
		$mobile = $r['mobile'];
	} else {
		if($option == 'username' || $option == 'mobile') {
			$mobile = $username;
			if($option == 'username' && $MOD['mobile']) {
				$r = $db->get_one("SELECT username FROM {$DT_PRE}member WHERE `mobile`='$username'");
